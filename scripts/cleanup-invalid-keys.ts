...
const EXTRACTED_KEYS_FILE = path.join(
    ROOT_DIR,
    '.cache/extracted-i18n-keys.json'
)
...
async function cleanupInvalidKeys(): Promise<void> {
    ...
    console.log(
        `Found ${availableLanguages.length} language directories: ${availableLanguages.join(', ')}`
    )
    ...
    for (const lang of availableLanguages) {
        ...
        const invalidKeys = existingKeys.filter(
