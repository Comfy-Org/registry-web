...
async function extractKeysFromFile(filePath: string): Promise<ExtractedKey[]> {
  try {
    ...
    matches.forEach((match) => {
      const key = match[2];
      if (key.includes("${"))
        throw new Error(`Dynamic keys are not supported: ${key} in file ${filePath}`);
      extractedKeys.push({
        key,
        file: filePath,
      });
...
async function readJsonFile(filePath: string): Promise<Record<string, string>> {
  try {
    ...
    try {
      ...
      if (content.includes("<<<<<<<")) {
        return JSON.parse(content.replaceAll(/^<<<<<<< .*$|^=======$|^>>>>>>> .*$/gm, ""));
      }
...
async function translateKeyToLanguage(
  ...
  try {
    ...
    throw new Error(`Error translating key "${key}" to ${lang}: ${String(error)}`);
  }
}
...
async function updateLocaleFiles(uniqueKeys: string[]): Promise<void> {
  ...
  for (const lang of availableLanguages) {
    ...
    for (const key of newKeysLang) {
      const translation = await translateKeyToLanguage(key, lang, updatedLangTranslations);
