...
function ClaimMyNodePage() {
  ...
  const [currentStage, setCurrentStage] = useState<ClaimStage>("info_confirmation");
  ...
  const { data: publisherToClaim, isLoading: publisherLoading } = useGetPublisher(
    ...
    mutation: {
      onSuccess: () => {
        ...
        [
          ...
          getListNodesForPublisherV2QueryKey(UNCLAIMED_ADMIN_PUBLISHER_ID),
      ...
      try {
        ...
        try {
          ...
          return {
            hasPermission: false,
            userInfo: undefined,
            errorMessage: t("Failed to get GitHub user information. Please try again."),
          };
        }
  ...
  useEffect(() => {
    ...
    if (node.publisher?.id !== UNCLAIMED_ADMIN_PUBLISHER_ID) {
      setError(t("This node is already claimed and cannot be claimed again."));
      return;
    }
      ...
      verifyRepoPermissions(token, owner, repo, nodeIdParam)
        .then((result) => {
          if (result.hasPermission) {
            ...
            const errorMsg =
              result.errorMessage ||
              t("Unable to verify repository permissions. Please try again.");
            setError(errorMsg);
  ...
  const handleClaimNode = () => {
    if (!isVerified || !githubToken) {
      toast.error(t("GitHub verification is required before claiming the node."));
      return;
    }
  ...
  return (
    <div className="container p-6 mx-auto">
      ...
      <div className="flex items-center cursor-pointer mb-8" onClick={handleGoBack}>
        ...
        <span className="text-gray-400 pl-1 text-base">{t("Back to node details")}</span>
      </div>
      ...
      <div className="bg-gray-800 p-6 rounded-lg mb-6">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-lg font-medium text-white">{t("Claim Process")}</h3>
          </div>
            ...
            <div className="flex mb-2 relative z-10">
              {[
                ...
                <div key={stage} className="flex-1 text-center">
                  ...
                  >
                    {stage === "info_confirmation" && t("Info")}
                    {stage === "github_login" && t("GitHub Login")}
                    {stage === "verifying_admin" && t("Verify Admin")}
        ...
        {currentStage === "info_confirmation" && (
          <div>
            ...
            <div className="bg-gray-700 p-4 rounded-lg mb-6">
              <h3 className="text-lg font-medium text-white mb-2">{t("Node Information")}</h3>
              <div className="flex flex-col space-y-2">
                <div className="flex items-start">
                  <span className="text-gray-400 w-32 ">{t("Node")}:</span>
                  <NodeSpan
                ...
                <div className="flex items-start">
                  <span className="text-gray-400 w-32 ">{t("Repository")}:</span>
                  <span className="text-white break-all">{node?.repository}</span>
                </div>
                <div className="flex items-start">
                  <span className="text-gray-400 w-32 ">{t("Publisher")}:</span>
          ...
          <div>
            ...
            <div className="bg-gray-700 p-4 rounded-lg mb-6 flex items-center justify-center">
              <div className="text-center py-8">
                ...
                <p className="text-white">{t("Redirecting to GitHub for authentication...")}</p>
              ...
              <div className="text-center py-8">
                {permissionCheckLoading ? (
                  ...
                  <>
                    ...
                    <p className="text-white">{t("Processing verification result...")}</p>
                  </>
                )}
              </div>
            </div>
        ...
        {currentStage === "claim_node" && (
          <div>
            <h2 className="text-xl font-bold text-white mb-4">{t("Step 4: Claim Your Node")}</h2>
            <div className="bg-gray-700 p-4 rounded-lg mb-6">
              <div className="flex items-center mb-4">
                <HiCheckCircle className="w-5 h-5 text-green-500 mr-2" />
                <h3 className="text-lg font-medium text-white">{t("Verification Successful")}</h3>
              </div>
              <p className="text-gray-300 mb-4">
                {githubUsername
                  ? t(
              ...
              <div className="flex justify-end">
                <Button color="blue" onClick={handleClaimNode} disabled={isClaimingNode}>
                  {isClaimingNode ? (
                    <>
        ...
        {currentStage === "completed" && (
          <div>
            <h2 className="text-xl font-bold text-white mb-4">{t("Step 5: Claim Successful")}</h2>
            <div className="bg-gray-700 p-4 rounded-lg mb-6">
              <div className="flex items-center mb-4">
                <HiCheckCircle className="w-5 h-5 text-green-500 mr-2" />
                <h3 className="text-lg font-medium text-white">{t("Node Claimed Successfully")}</h3>
              </div>
              ...
              <div className="flex justify-end">
                <Button color="blue" onClick={handleGoToNodePage}>
