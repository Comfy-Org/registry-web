...
const NodeTranslationEditor = () => {
  ...
  if (!canEdit) {
    return (
      <div className="p-4">
        <Alert color="warning">
          {t('You do not have permission to edit translations for this node.')}
        </Alert>
      </div>
    )
  }
      ...
      <Card>
        ...
        <div className="mb-4 border-b border-gray-200 dark:border-gray-700 pb-4">
          ...
          <div className="flex gap-2">
            <TextInput
              placeholder={t('Field name (e.g., description, category)')}
              value={newFieldKey}
        ...
        <div className="space-y-4">
          {Array.from(allFields).map((field) => (
            ...
            >
              ...
              {field === 'description' ? (
                <Textarea
                  value={getFieldValue(currentTranslations, field)}
                  onChange={(e) => updateTranslation(field, e.target.value)}
                  rows={3}
                  placeholder={t('Enter field translation...')}
                />
              ) : (
                <TextInput
                  value={getFieldValue(currentTranslations, field)}
              ...
              {existingTranslations[selectedLanguage]?.[field] ? (
                <p className="text-xs text-gray-500 mt-1">
                  {t('Original')}:{' '}
                  {getFieldValue(
                    existingTranslations[selectedLanguage] as Record<
                      string,
                      string | string[]
                    >,
                    field
