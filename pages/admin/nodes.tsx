...
import { Breadcrumb, Button, Label, Modal, Spinner, Table, TextInput } from "flowbite-react";
...
function NodeList() {
  ...
  const defaultSelectedStatuses = [(router.query as any)?.status ?? Object.keys(statusFlags)]
    .flat()
    .map((status) => statusFlags[status])
    .filter(Boolean);

  const [selectedStatuses, _setSelectedStatuses] = React.useState<NodeStatus[]>(
    defaultSelectedStatuses.length > 0 ? defaultSelectedStatuses : [NodeStatus.NodeStatusActive],
  );
  ...
  const setSelectedStatuses = (statuses: NodeStatus[]) => {
    ...
    const checkedAll =
      allStatuses.join(",").toString() === [...statuses].sort().join(",").toString();
  ...
  const handleJwtTokenGenerated = async () => {
    ...
    if (pendingBanOperation) {
      await executeBanOperation(pendingBanOperation.node, pendingBanOperation.action);
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-4 mb-6">
        <h1 className="text-2xl font-bold text-gray-200">{t("Node Management")}</h1>
        ...
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            const inputElement = document.getElementById("filter-node-id") as HTMLInputElement;
        ...
        <div className="flex gap-2 flex-wrap">
          <Button
            color={
              selectedStatuses.length >= Object.keys(statusFlags).length ? statusColors.all : "gray"
            }
            className={clsx({
              "brightness-50": !(selectedStatuses.length >= Object.keys(statusFlags).length),
              "hover:brightness-100": !(selectedStatuses.length >= Object.keys(statusFlags).length),
                ...
                <Table.Cell className="dark">
                  {node.publisher?.name && (
                    <div>
                      ...
                      <div className="text-sm text-gray-500">{node.publisher.id}</div>
                    </div>
                  )}
                </Table.Cell>
                  ...
                  <div className="flex gap-2">
                    ...
                    {node.status === NodeStatus.NodeStatusActive && (
                      <Button
                        ...
                        disabled={!node.publisher?.id || processingNodeId === node.id}
                        title={t("Ban this node")}
                      >
                        {processingNodeId === node.id ? <Spinner size="sm" /> : t("Ban")}
                      </Button>
                    )}
                    ...
                    {node.status === NodeStatus.NodeStatusBanned && (
                      ...
                      >
                        {processingNodeId === node.id ? <Spinner size="sm" /> : t("Unban")}
                      </Button>
                    )}
        ...
        <Modal.Body className="dark">
          <div className="space-y-6" onKeyDown={handleKeyDown}>
            ...
            <div>
              ...
              {/* Predefined Tags */}
              <div className="mb-3">
                <div className="text-sm text-gray-400 mb-2">{t("Quick Add Tags")}:</div>
      ...
      <Modal
        ...
        className="dark"
      >
        <Modal.Header className="dark">{t("Confirm Unclaim Node")}</Modal.Header>
        <Modal.Body className="dark">
          <div className="space-y-4">
            ...
            <div className="bg-gray-700 p-4 rounded">
              <div className="font-semibold text-white">{unclaimingNode?.name}</div>
        ...
        <Modal.Footer className="dark">
          ...
          >
            {adminUpdateNodeMutation.isPending ? <Spinner size="sm" /> : t("Unclaim Node")}
          </Button>
