...
/**
 * Indicates where a translation came from so the UI can label it accurately:
 * - "original": English source (locale === "en") or fallback when no translation exists
 * - "stored": publisher-provided translation loaded from `node.translations[locale]`
 * - "auto": generated on-the-fly by OpenAI in `translateNodeContent`
 */
export type TranslationSource = "original" | "stored" | "auto";
...
export function getTranslatedNodeContent(node: Node, locale: string): TranslatedNodeContent {
  ...
  const translation = node.translations?.[locale] as { description?: unknown } | undefined;

  const translatedDesc = translation?.description;
  const hasStored = typeof translatedDesc === "string" && !!translatedDesc;

  return {
    locale,
    description: hasStored ? (translatedDesc as string) : node.description,
    source: hasStored ? "stored" : "original",
  };
}

/**
 * Auto-translate missing content using OpenAI API (server-side only).
 * Used by both getStaticProps (ISR-time, blocking, cached for revalidate window)
 * and the /api/translate-node route (async client-side fallback).
...
export async function translateNodeContent(
  ...
  const langName = (LANGUAGE_NAMES as Record<string, string>)[content.locale] ?? content.locale;
  ...
  try {
    ...
    if (match?.[1]?.trim()) {
      return { ...content, description: match[1].trim(), source: "auto" };
    }
    return content;
  } catch {
    return content;
