...
export default function SearchRankingEditModal({
  ...
  const [searchRanking, setSearchRanking] = useState<number>(defaultSearchRanking);
  ...
  const updateNodeMutation = useUpdateNode({
    mutation: {
      ...
      onError: (error: AxiosError<Error>) => {
        toast.error(error.response?.data?.message || t("Failed to update search ranking"));
      },
    },
  });
  ...
  const onSubmit: FormEventHandler = async (e) => {
    ...
    qc.setQueryData(
      ...
      (oldData: Node | undefined) => oldData && { ...oldData, search_ranking: searchRanking },
    );
    qc.setQueriesData(
      {
        ...
        return (
          oldData && {
            ...
            nodes: oldData?.nodes?.map((n) =>
              n.id === nodeId ? { ...n, search_ranking: searchRanking } : n,
            ),
          }
        );
      },
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                value={searchRanking}
                onChange={(e) =>
                  setSearchRanking(Math.max(1, Math.min(10, parseInt(e.target.value) || 1)))
                }
