...
import type { TranslatedNodeContent, TranslationSource } from "@/src/hooks/i18n/translateNode";
...
const NodeDetails = ({
  ...
  const [asyncTranslation, setAsyncTranslation] = useState<{
    description: string;
  ...
  // Async-fetch translation if ISR didn't have one (avoids blocking getStaticProps)
  useEffect(() => {
    // Always reset stale translation from previous (node, locale) before any early-return
    setAsyncTranslation(null);
    setIsTranslating(false);
    const locale = i18n.language;
    if (!node?.id || locale === "en") return;
    // Skip if we already have a matching translation from ISR
    if (translatedContent?.locale === locale && translatedContent?.description !== node.description)
      return;
    setIsTranslating(true);
    const controller = new AbortController();
    fetch(`/api/translate-node?nodeId=${encodeURIComponent(node.id)}&locale=${locale}`, {
      signal: controller.signal,
    })
      .then((r) => (r.ok ? r.json() : null))
      .then((data) => {
        // Only adopt the async result if it's a real translation, not the
        // English fallback the API returns when OpenAI/stored both fail.
        if (
          data?.description &&
          data.description !== node.description &&
          (data.source === "stored" || data.source === "auto")
        ) {
          setAsyncTranslation({
  ...
  }, [node?.id, node?.description, i18n.language, translatedContent]);
                ...
                {(() => {
                  const isrTranslation =
                    ...
                    translatedContent.description !== node.description
                      ...
                      : null;
                  const activeTranslation = asyncTranslation ?? isrTranslation;
                  return (
                    <ContentToggle
                      original={node.description ?? ""}
                      translated={activeTranslation?.description ?? null}
                      translationSource={activeTranslation?.source}
                      locale={i18n.language}
