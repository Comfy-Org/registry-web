...
const NodeDetails = ({
  ...
  const publisherId =
    node?.publisher?.id ?? (_publisherId ? String(_publisherId) : undefined) // try use _publisherId from url while useGetNode is loading
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/translate-node?nodeId=${encodeURIComponent(node.id)}&locale=${locale}`,
      {
