...
const Hit: React.FC<HitProps> = ({ hit }) => {
  ...
  return (
    ...
    >
        ...
        {hit.comfy_nodes?.length && (
          <div className="mb-1 text-xs font-light text-white mt-2 flex-row flex gap-2 whitespace-nowrap overflow-hidden overflow-ellipsis">
            ...
            >
              {matchedNodes?.length ? (
                <>
                  {matchedNodes?.length ?? 0}/{hit.comfy_nodes?.length ?? 0} Nodes matched:
                  {matchedNodes
                    ?.map((e) => e.value?.replace(/<\/?mark>/g, "**"))
                    .filter((e) => e)
                    .map((name) => (
                      <div key={name}>
      ...
      {/* meta info */}
      <p className="text-xs font-light text-nowrap mt-2 text-gray-400" dir="ltr">
        ...
        {hit.total_install && (
          <span className="">
            {" | "}
            <HiDownload className="inline h-3 w-3" title={t("Installs")} />{" "}
        ...
        {hit.github_stars && hit.github_stars > 0 && (
          <span className="">
            {" | "}
            <HiStar className="inline h-3 w-3" title={t("GitHub Stars")} />{" "}
