...
const CreatePublisherFormContent: React.FC<CreatePublisherFormContentProps> = ({
  ...
  return (
    <>
      {showTitle && <h2 className="text-2xl font-bold text-white mb-4">{t("Create Publisher")}</h2>}
      ...
      <form className="mt-4 space-y-4 lg:space-y-6" onSubmit={handleSubmit}>
        <div>
          <label className="block mb-1 text-xs font-bold text-white">{t("Username")}</label>
          <TextInput
            id="name"
            ...
            color={!isLoadingValidation && publisherValidationError ? "failure" : "success"}
            helperText={
              <>
                {isLoadingValidation && <>{t("Checking username...")}</>}
                {!isLoadingValidation && publisherValidationError && (
                  <>
                    <span className="font-medium"></span> {publisherValidationError}
                  </>
                )}
              </>
            }
        ...
        <div>
          <label className="block mb-1 text-xs font-bold text-white">{t("Display Name")}</label>
          <TextInput
            sizing="sm"
            theme={customThemeTextInput}
            id="displayName"
        ...
        <div className="flex center gap-4">
          <Button type="button" onClick={onCancel} color="light" className=" bg-gray-900">
