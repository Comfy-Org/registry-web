...
export default function ProfileDropdown() {
  ...
  return (
    ...
    >
      <Dropdown.Header>
        <span className="block text-sm">{firebaseUser.displayName || t("User")}</span>
        <span className="block truncate text-sm font-medium">{firebaseUser.email}</span>
      </Dropdown.Header>
      <Dropdown.Item onClick={() => router.push("/nodes")}>{t("Your Nodes")}</Dropdown.Item>
      {user?.isAdmin && (
        <Dropdown.Item onClick={() => router.push("/admin")}>{t("Admin Dashboard")}</Dropdown.Item>
      )}
      <Dropdown.Divider />
