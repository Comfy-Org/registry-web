...
export function NodeStatusReason(nv: NodeVersion) {
  ...
  const { fullfilledIssueList, lastFullfilledIssueList } = useMemo(() => {
    ...
    const addIssueUrl = (issue: z.infer<typeof zErrorArray>[number], repoUrl: string) => {
      const filepath =
...
export function PrettieredYAML({ children: raw }: { children: string }) {
  ...
  useEffect(() => {
    let cancelled = false;
    formatCode(parsedYaml, "yaml")
      .then((formatted) => {
        if (!cancelled) setCode(formatted);
      })
      .catch(() => {
        if (!cancelled) setCode(parsedYaml);
      });
    return () => {
      cancelled = true;
    };
...
export function PrettieredYamlDiffView({
  ...
  useEffect(() => {
    ...
    Promise.all([formatCode(parsedOriginal, "yaml"), formatCode(parsedModified, "yaml")])
      ...
      .catch(() => {
        if (!cancelled) {
          setCodeOriginal(parsedOriginal);
          setCodeModified(parsedModified);
        }
      });
    return () => {
      cancelled = true;
